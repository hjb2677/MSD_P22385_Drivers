...
// File    : SchedulerPrototype.c
// Author  : Harrison Barnes
// Data    : 05/03/2022
// Purpose : Prototype driver program for the scheduler
...
// Function : main
// Inputs   : argc, argv - cmd line args
// Outputs  : return code of program
//
// main controls the scheduler prototype by testing time of day and waiting
// Prog flow - fetch time -> validate -> wait time
int main(int argc, char *argv[]) {
    // Var holds time of day fetched from internet
    time_t timeOfDay = FetchTime();

    // Test the PrintTimeOfDay function
    printf("Testing PrintTimeOfDay:\n");
    PrintTimeOfDay(timeOfDay);

    // Validate time, because why not right?
    int validCode = ValidateTime(timeOfDay);
    printf("\nValidate returned %d", validCode);

    return 0; // Return 0 to designate success
}
